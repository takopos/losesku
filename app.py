...
# --- 讀取完整商品庫 (已加入強制純文字防呆機制) ---
@st.cache_data(ttl=600)
def load_master_data():
    try:
        # 加入 dtype=str，強制將 Excel 內所有內容視為純文字，避免條碼開頭的 0 消失或變成小數點
        df = pd.read_excel(GDRIVE_DIRECT_URL, engine='openpyxl', dtype=str)
        if '助記碼' in df.columns:
            # 去除頭尾空白，並用正則表達式把萬一出現的 ".0" 小數點結尾砍掉
            df['助記碼'] = df['助記碼'].astype(str).str.strip().str.replace(r'\.0$', '', regex=True)
        return df
...
if master_df is not None:
    ...
    with tab1:
        ...
        enable_camera = st.toggle("📸 開啟相機鏡頭")
        
        if enable_camera:
            camera_image = st.camera_input("請對準條碼並點擊拍照")
                ...
                if decoded_objects:
                    ...
                    if new_codes:
                        ...
                        if success:
                            ...
                            st.error("❌ 傳送失敗，請檢查 API 連線。")
                else:
                    st.error("❌ 找不到條碼，請確認畫面清晰再拍一次。")
        else:
            st.warning("👈 請點擊上方開關以啟動相機。")
    ...
    with tab4:
        ...
        if uploaded_file:
            # 這裡也加上 dtype=str 防呆，確保手動上傳的條碼不會被當成數字
            if uploaded_file.name.endswith('.xlsx'):
                scan_df = pd.read_excel(uploaded_file, dtype=str)
            else:
                scan_df = pd.read_csv(uploaded_file, dtype=str)
                
            scan_col = st.selectbox("請選擇包含「掃描條碼」的欄位", scan_df.columns)
            
            if st.button("⬆️ 將這批條碼加入暫存庫"):
                # 確保轉為字串並去掉可能的 .0
                batch_codes = scan_df[scan_col].astype(str).str.strip().str.replace(r'\.0$', '', regex=True).tolist()
        ...
        if 'processing_codes' in st.session_state and st.session_state.processing_codes:
            # 清理 API 抓回來的條碼，確保格式乾淨
            final_codes = list(set([str(c).strip().replace('.0', '') for c in st.session_state.processing_codes if str(c).strip()]))
